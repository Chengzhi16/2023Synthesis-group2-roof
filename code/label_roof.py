...
parser.add_argument('--path', '-p', type=str, default='denhaag_roof.geojson', help='An optional path argument')
...
import pandas as pd
#features = pd.read_csv("rotterdam_roof_features_1028a.csv")

features = pd.read_csv("denhaag_roof_features_1028a.csv")
 ...
 #   random_value = random.random()
  ...
  #  if random_value > probability:
   #     continue
    ...
    N_horizontal = group["NumberofHorzontial"]

    Intersection_angle_1_2 = group["intersection_angle_max_min"]

    Relative_H = group["Relative_Max_Height (Maximum roof height - minimum)"]

    Max_area_p = group["MaxAreaPropotion_x"]

    Max_polygon_angle = group["LargestPolygonAngle_x"]
    Small_parts_area_p = group["The area proportion of small roof parts"]
    N_small_parts = group["The number of small roof parts"]
    ...
    def plot(maxx, minx, maxy, miny, maxz, minz,polygons,polygons_surroundings,
        ...
        textsr = f"City_label: {label_file} \n Building ID: {building_id} \n Max_polygon_proportion: {Max_area_p: .4f} \n Angle_1_2: {Intersection_angle_1_2:.3f}  \
                                 \n Relative_Height: {Relative_H:.3f} \n  Max_polygon_angle: {Max_polygon_angle:.3f} \n N_horzontial: {N_horizontal} \
                                 \n Small_parts_proportion: {Small_parts_area_p:.3f} \n N_small_parts: {N_small_parts}"
